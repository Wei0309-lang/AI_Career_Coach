...
**AI／雲端服務**：Google Gemini（對話與解析）、Azure Speech（3D 模式語音合成）、LiveAvatar/HeyGen（擬真虛擬人）、Supabase Auth（登入/註冊）
...
| Ollama | 最新版 | AI 面試功能（選用，可用 `USE_GEMINI_CHAT=1` 跳過） |
...
| `USE_GEMINI_CHAT` | 選填 | 設為 `1` 時本地以 Gemini 取代 Ollama，免安裝本地模型 |
...
### 7. Ollama 模型設定（AI 面試功能，選用）

> 此步驟僅影響 `/chat` AI 面試功能。若 `.env` 設定 `USE_GEMINI_CHAT=1` 可跳過本步驟。
...
代表本機 `app.db` 是舊版 schema（帶有 `is_verified`、`password_hash` 等欄位）。呼叫以下網址重置本機資料庫即可（只會清空本機測試資料，**不會影響 Supabase 帳號**，不需要重新註冊）：
